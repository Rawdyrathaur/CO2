...
export default function RecommendedActions() {
  ...
  return (
    <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-lg sm:rounded-xl md:rounded-2xl p-4 sm:p-5 md:p-6 lg:p-8 border border-gray-800/50">
      <div className="mb-4 sm:mb-5 md:mb-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-0">
          <div className="flex-1">
            <h3 className="text-white font-bold text-base sm:text-lg">Carbon Optimizations</h3>
            <p className="text-gray-500 text-xs sm:text-sm mt-1">Apply actions to reduce emissions</p>
          </div>
          {hasOptimizations && (
            <div className="px-2.5 sm:px-3 py-1 sm:py-1.5 bg-green-900/30 border border-green-600/40 rounded-full flex-shrink-0">
              <span className="text-[10px] sm:text-xs font-bold text-green-400">{activeCount} Active</span>
            </div>
          )}
        </div>
      </div>
      ...
      {hasOptimizations && (
        <div className="mb-4 sm:mb-5 md:mb-6 relative overflow-hidden bg-gradient-to-br from-green-900/30 via-green-800/20 to-emerald-900/30 border border-green-600/40 rounded-lg sm:rounded-xl md:rounded-2xl p-3 sm:p-4 md:p-5 lg:p-6">
          ...
          <div className="relative">
            <div className="flex items-center gap-2 mb-3 sm:mb-4">
              <div className="p-1.5 sm:p-2 bg-green-600/20 rounded-lg">
                <FiTrendingDown className="text-green-400" size={16} />
              </div>
              <div>
                <h4 className="text-green-400 font-bold text-sm sm:text-base">Live Impact</h4>
                <p className="text-green-400/60 text-[10px] sm:text-xs">Optimizations reducing your footprint</p>
              </div>
            </div>

            <div className="mb-3 sm:mb-4">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[10px] sm:text-xs text-gray-400 font-medium">Reduction Level</span>
                <span className="text-base sm:text-lg font-black text-green-400">{totalReduction.toFixed(1)}%</span>
              </div>
              <div className="h-2 sm:h-3 bg-gray-800/50 rounded-full overflow-hidden border border-gray-700/30">
                <div
            ...
            <div className="grid grid-cols-2 gap-2 sm:gap-3 md:gap-4">
              <div className="bg-gray-900/40 rounded-lg sm:rounded-xl p-2 sm:p-2.5 md:p-3 border border-green-600/20">
                <p className="text-[10px] sm:text-xs text-gray-400 mb-1 sm:mb-1.5 flex items-center gap-1">
                  <span className="w-1 h-1 sm:w-1.5 sm:h-1.5 bg-green-400 rounded-full"></span>
                  Carbon Saved
                </p>
                <p className="text-base sm:text-lg md:text-xl font-black text-green-400">
                  {savedCarbon < 0.001
              ...
              <div className="bg-gray-900/40 rounded-lg sm:rounded-xl p-2 sm:p-2.5 md:p-3 border border-green-600/20">
                ...
                <p className="text-base sm:text-lg md:text-xl font-black text-emerald-400">
            ...
            <div className="mt-3 sm:mt-4 pt-3 sm:pt-4 border-t border-green-600/20">
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-0">
                ...
                <div className="flex items-center gap-1 px-2 py-1 bg-green-600/20 rounded">
                  <FiTrendingDown size={10} className="text-green-400" />
      ...
      <div className="space-y-2 sm:space-y-3">
        {actions.map((action) => {
          ...
          return (
            <div
              key={action.id}
              className={`group relative rounded-lg sm:rounded-xl p-3 sm:p-4 md:p-5 border transition-all duration-300 ${
                isApplied
            ...
            >
              {isApplied && (
                <div className="absolute top-2 sm:top-3 right-2 sm:right-3">
                  <div className="flex items-center gap-1 sm:gap-1.5 px-1.5 sm:px-2 py-0.5 sm:py-1 bg-green-600/30 border border-green-500/30 rounded-full">
                    <div className="w-1 h-1 sm:w-1.5 sm:h-1.5 bg-green-400 rounded-full animate-pulse"></div>
                    <span className="text-[10px] sm:text-xs font-semibold text-green-400">Active</span>
                  </div>
                </div>
              )}
              ...
              <div className="flex items-start gap-2 sm:gap-3 md:gap-4">
                <div className={`p-2 sm:p-2.5 md:p-3 rounded-lg sm:rounded-xl transition-all duration-300 flex-shrink-0 ${
                  ...
                  <Icon className={`transition-colors ${isApplied ? 'text-green-400' : 'text-accent'}`} size={16} />
                </div>

                <div className="flex-1 min-w-0 pt-0.5">
                  <h4 className="text-white font-bold text-xs sm:text-sm mb-1 sm:mb-1.5 pr-16 sm:pr-0">{action.title}</h4>
                  <p className="text-gray-400 text-[10px] sm:text-xs leading-relaxed mb-2 sm:mb-3">{action.description}</p>

                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-3">
                    <div className={`inline-flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1 sm:py-1.5 rounded-lg ${
                      isApplied
                        ? 'bg-green-600/20 border border-green-600/30'
                        : 'bg-accent/10 border border-accent/20'
                    }`}>
                      <FiTrendingDown size={10} className={isApplied ? 'text-green-400' : 'text-accent'} />
                      <span className={`text-[10px] sm:text-xs font-bold ${isApplied ? 'text-green-400' : 'text-accent'}`}>
                    ...
                    <button
                      ...
                      className={`w-full sm:w-auto sm:ml-auto px-3 sm:px-4 py-1.5 sm:py-2 font-bold text-[10px] sm:text-xs rounded-lg transition-all duration-300 whitespace-nowrap ${
                        isApplied
      ...
      <div className="mt-3 sm:mt-4">
        <button
          ...
          className="w-full flex items-center justify-between p-2.5 sm:p-3 bg-gray-800/20 hover:bg-gray-800/30 border border-gray-700/30 rounded-lg transition-all"
        >
          <div className="flex items-center gap-1.5 sm:gap-2">
            <FiInfo size={12} className="text-gray-400" />
            <span className="text-[10px] sm:text-xs font-semibold text-gray-400">Best Practices</span>
          </div>
          {showTips ? (
            <FiChevronUp size={12} className="text-gray-500" />
          ) : (
        ...
        {showTips && (
          <div className="mt-2 sm:mt-3 p-3 sm:p-4 bg-gray-800/20 border border-gray-700/30 rounded-lg">
            <div className="space-y-2 sm:space-y-3 text-[10px] sm:text-xs text-gray-400">
              <div>
                <p className="text-white font-semibold mb-1 sm:mb-1.5">Message Efficiency</p>
              ...
              <div>
                <p className="text-white font-semibold mb-1 sm:mb-1.5">Timing Optimization</p>
                <ul className="space-y-0.5 sm:space-y-1 ml-3 sm:ml-4">
              ...
              <div>
                <p className="text-white font-semibold mb-1 sm:mb-1.5">Infrastructure</p>
                <ul className="space-y-0.5 sm:space-y-1 ml-3 sm:ml-4">
